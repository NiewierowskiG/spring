...
type RestOf<T extends IEmail | NewEmail> = Omit<T, 'sendingDate'> & {
...
export class EmailService {
  ...
  protected convertDateFromClient<T extends IEmail | NewEmail | PartialUpdateEmail>(email: T): RestOf<T> {
    return {
      ...email,
      sendingDate: email.sendingDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmail: RestEmail): IEmail {
    return {
      ...restEmail,
      sendingDate: restEmail.sendingDate ? dayjs(restEmail.sendingDate) : undefined,
    };
  }
